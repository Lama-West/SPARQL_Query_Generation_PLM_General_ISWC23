...
class T5Seq2Seq(torch.nn.Module):
  def __init__(self, voc_size, unk_token, t5, copy_layer):
    super().__init__()
    self.voc_size = voc_size
    self.unk_token = unk_token
    self.t5 = t5
    self.t5.resize_token_embeddings(voc_size)
    self.copy_layer = copy_layer
    if copy_layer:
      self.criterion = nn.CrossEntropyLoss(label_smoothing=0.1)
    else:
      self.criterion = None
    self.optimizer=optim.AdamW(self.parameters(),lr=0.0015)
  ...
  def forward(self, input_ids, trg_ids, mask, generation=False):
    ...
    labels = trg_ids
    if generation:
        output_dict = self.t5(input_ids=input_ids.masked_fill(input_ids >= self.voc_size, self.unk_token), 
                                attention_mask=mask,
  ...
  def generate(self, src, mask, max_length, use_copy, sos_token):
    ...
    if use_copy:
      ...
      return self.t5.generate(input_ids=src, num_beams=1, attention_mask=mask, early_stopping=True, max_length=max_length,num_return_sequences=1)[:,1:]

  def beam_generate(self, src, mask, max_length, num_beams, num_returns):
    return self.t5.generate(input_ids=src, num_beams=num_beams, attention_mask=mask, early_stopping=True, max_length=max_length,num_return_sequences=num_returns)[:,1:]
  ...
  def create_model(config, use_copy, device):
    try: t5 = T5ForConditionalGeneration.from_pretrained("t5-small")
    except: t5 = T5ForConditionalGeneration.from_pretrained("t5")
    copy_layer = CopyLayerVocabExtend(config["voc_size"]) if use_copy else None
    return T5Seq2Seq(config["voc_size"], config["unk_token"], t5, copy_layer)